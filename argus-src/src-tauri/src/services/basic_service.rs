...
pub fn get_basic_setting() -> Result<PhotoStorage, SqlError> {
    let mut conn = establish_connection();
    storage::photo_storage::get_basic_setting(&mut conn)
}
...
pub fn update_basic_setting_img_path(img_paths: String) -> Result<(), SqlError> {
    ...
    // storage::photo_storage::update_basic_setting(&mut conn, &mut basic_setting)
    Ok(())
}
/// 添加一个图片路径
